## This is the Fork Version of [FAST-LIO2](https://github.com/hku-mars/FAST_LIO)
There are Two Additional Function over [FAST-LIO2](https://github.com/hku-mars/FAST_LIO)

**Multi-LiDAR Calibration:**

1. Export a Global Prior Map by set ```pcd_save_en: true    pcd_file_name: "PSAPrimeMover" interval: 10 ``` in ```ouster32.yaml```
```
roslaunch fast_lio mapping_ouster32.launch
```
2. Load the Global Prior Map (set ```base_map_file_path``` as ```PSAPrimeMover.pcd```). Calculate extrinsic roughly through the PCL::GICP and export the extrinsic (```psacalibr_pose_n.pcd```) and its corresponding pointcloud (```psacalibr_n.pcd```). 
```
roslaunch fast_lio map_merge.launch
```
3. Optimize the extrinsic using Lidar BA ([BALM2](https://github.com/hku-mars/BALM)). Set ```point_range=1000```, 
```pcd_file_name="psacalibr_"``` in ```balm.launch```
```
...
roslaunch fast_lio mapping_ouster32.launch
```
2. Optimize the pose using Lidar BA ([BALM2](https://github.com/hku-mars/BALM)). Set ```point_range=10```, 
```pcd_file_name="PSAPrimeMover_"``` in ```balm.launch```
```
roslaunch fast_lio balm.launch
```
