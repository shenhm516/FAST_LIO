...
int read_pose(vector<double> &tims, PLM(3) &rots, PLV(3) &poss, string prename) {
  for(int m=0; m<600; m++) {
    string filename = prename + pcd_file_name + "pose_" + to_string(m) + ".pcd"; // for truck model
    // string filename = prename + "psa_pose" + to_string(m) + ".pcd";
    pcl::PointCloud<PointXYZISE3T>::Ptr pl_ptr(new pcl::PointCloud<PointXYZISE3T>());
    pcl::PointCloud<PointXYZISE3T> pl_tem;
    if(pcl::io::loadPCDFile(filename, pl_tem)==-1) return m;
    if(pl_tem.size()!=1) return m; 
    
    Eigen::Matrix<double, 3, 1> pos;
    pos(0,0) = pl_tem.points[0].x; 
    pos(1,0) = pl_tem.points[0].y;
    pos(2,0) = pl_tem.points[0].z;
    // std::cout << pos << std::endl;
    poss.push_back(pos);
    rots.push_back(Exp(Eigen::Vector3d(pl_tem.points[0].rx,pl_tem.points[0].ry,pl_tem.points[0].rz)));
    if(m==599) return m;
  }
}
...
void read_file(vector<IMUST> &x_buf, vector<pcl::PointCloud<PointType>::Ptr> &pl_fulls, string &prename)
{
  ...
  // std::cout << pose_size << std::endl;
  for(int m=0; m<pose_size; m++) {
    string filename = prename + pcd_file_name + to_string(m) + ".pcd"; // for truck model
    // string filename = prename + "psa_scan" + to_string(m) + ".pcd";
    ...
    for(pcl::PointXYZI &pp: pl_tem.points) {
      ...
      if (p.norm()<point_range && p.norm()>0.2) // for truck model
        pl_ptr->push_back(ap);
    }
...
void data_show(vector<IMUST> x_buf, vector<pcl::PointCloud<PointType>::Ptr> &pl_fulls)
{
  ...
  // std::cout << x_buf[winsize].p.transpose() << std::endl;
...
int main(int argc, char **argv)
{
  ...
  pub_path = n.advertise<sensor_msgs::PointCloud2>("/map_path", 100, true);
  pub_show = n.advertise<sensor_msgs::PointCloud2>("/map_show", 100, true);
  ...
  n.param<double>("point_range", point_range, 10);
  n.param<string>("pcd_file_name", pcd_file_name, "");
  
  
  string file_path = std::string(ROOT_DIR);
  ...
  { 
    ...
    if (x_buf.size()<10) {
      for (int i=0; i<x_buf.size(); i++) {
        std::cout << "The position " << std::to_string(i) << " is:" << std::endl;
        std::cout << x_buf[i].p.transpose() << std::endl;
        std::cout << "The rotation " << std::to_string(i) << " is:" << std::endl;
        std::cout << x_buf[i].R << std::endl;
      }
    }
